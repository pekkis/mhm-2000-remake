...
| KARSA.M2K            | **lineup slot mask** → `dad(1..5, 1..6)` — 5 skater slots × 6 lineup configurations (1 = slot required, 0 = optional). File rows = lineup configs: rows 1–4 = the four regular lines, row 5 = power play (PP), row 6 = penalty kill (PK). File columns are read in QB order `[3,4,5,1,2]` (`dad(3), dad(4), dad(5), dad(1), dad(2)` per row). Resulting required-slot map: lines 1–3 = full 5 slots, **line 4 = `dad(1)=dad(2)=0`** (you must field 4 forward lines, but the 4th line's two D slots are optional — i.e. you can play with only 6 D total), **PP = full 5**, **PK = `dad(5)=0`** (4 skaters, the 5th slot drops). Loaded once at `MHM2K.BAS:744`; the only consumer is `haikka` in `SUB ketlaita` (`ILEX5.BAS:2629`) which flips the "VAILLINAINEN KETJUKOKOONPANO" warning when a _required_ slot is empty. **Row 7 (`2,2,3,4,5`) is stray** — the QB `FOR zz = 1 TO 6` loop never reads it; looks like leftover developer scratch (a partial column header or display-order hint). Etymology: probably short for `karsinta` ("qualifier / pruning") — the file that prunes which slots are mandatory.                                                                                                                                                                                                                  |
...
| ORGA.M2K, ORGASM.M2K | **starting-cash + organization templates per team-strength bin**. Read by `SUB orgamaar` (`MHM2K.BAS:2012`); team-strength index = `(sed + sedd + seddd) / 3`. Sets `raha(pv)` and other org-side fields when a human picks a team. **In MHM 2000 the new-game starting cash is therefore team-strength-derived, not difficulty-derived** — a regression vs. MHM 97 where difficulty drove starting money. (TODO: full row map.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| TASOT.M2K            | **CPU team strength table** — 58 rows × 3 cols (`maz`/`puz`/`hyz` = goalie/defence/attack). Indexed by `tazo(team)`. SUB `tasomaar` ([MHM2K.BAS:2188](../MHM2K.BAS), [ILEZ5.BAS:1832](../ILEZ5.BAS)) loads into `lvl(1..58) AS tazzo` and derives raw `mw`/`pw`/`hw` with per-season noise ±1/±2/±4. Table ported to [src/data/levels.ts](../../data/levels.ts) (`teamLevels`, `TeamLevelDefinition`); the realised-strength shape (`TeamStrength`) and the noise roll (`rollTeamStrength`) live in [src/services/levels.ts](../../services/levels.ts). Human-managed teams overwrite via `orgamaar` (not ported yet).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
