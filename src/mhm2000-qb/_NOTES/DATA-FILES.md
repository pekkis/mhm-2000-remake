...
| BUDGET.M2K           | **per-round budget cost cube** — 15 LONG values loaded at `MHM2K.BAS:949-953` into `valbh(1..3, 1..5, 1..5)`. First dim = budget tier from `vai(2, pv)` (1=easy, 2=med, 3=hard). Second dim = budget category 1..5 (training, intensity, scouting, junior pipeline, salary). Third dim = chosen slider 1..5. All values are negative (costs). Read every round inside `SUB rahanvelvoitteet` (`ILEX5.BAS:3563/3565/3571`). Tier 3, category 5 (salary) further multiplies by `lpl(pv)` per active player.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
