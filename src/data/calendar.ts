// MHM 2000 runtime calendar — the single source of truth.
// The runtime engine (gameMachine) reads `calendar` exclusively.
// The raw decode in `./mhm2000/calendar.ts` is kept as cross-reference.
...
export type TurnPhase = "action" | "event" | "news" | "gala";
...
export type CalendarEntry = {
  ...
  pranks: boolean;
  /** QB fitness recovery (+2/round for resting players). False on preseason/training (types 99/4). */
  fitnessRecovery: boolean;
  /** QB readiness drift (tre ±0.0025). Only on regular runkosarja gamedays (type 1). */
...
const entries: RawCalendarEntry[] = [
  ...
  // round 6 (KIERO.M2K row 7, type 4) — training match
  {
    ...
    tags: [
      ...
      "season_tickets",
      "training",
      "todo:wire-practice-competition"
    ]
  },
  ...
  // round 52 (KIERO.M2K row 53, type 1) — regular gameday 26 + mmkisaalku (WCh starts)
  {
    ...
    fitnessRecovery: true,
    readinessDrift: true,
    tags: ["preRound:wch-start", "todo:wire-world-championships"]
  },
  ...
  // round 56 (KIERO.M2K row 57, type 1) — regular gameday 29 — last round with market open
  {
    ...
    fitnessRecovery: true,
    readinessDrift: true,
    tags: ["last-round-with-market-open"]
  },
